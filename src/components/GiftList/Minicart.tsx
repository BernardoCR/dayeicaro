          ...
          <div className="fixed bottom-0 left-0 right-0 z-40 animate-fade-in-up shadow-xl md:static md:mb-4">
            ...
            <button
              ...
              <div className="space-y-4 p-4 pt-0 md:pt-6">
                ...
                <textarea
                  rows={4}
                  placeholder="Escreva uma mensagem (máx. 56 caracteres)"
                  onChange={(e: ChangeEvent<HTMLTextAreaElement>) =>
                    setMessage(e.target.value)
                  }
                  value={message}
                  className="w-full p-2 border border-joanGreen-600 rounded-lg my-4"
                  maxLength={56}
                />
              </div>
              <button
...
function MinicartFloatingButton({
  ...
  return (
    <button
      className="fixed bottom-4 right-4 flex h-16 w-16 items-center justify-center rounded-full border border-joanGreen-600 bg-white text-joanGreen-600 shadow-xl"
...
function Minicart(): JSX.Element {
  ...
  return (
    <>
      {paymentOpen && (
        <PaymentModal setPaymentOpen={setPaymentOpen} message={message} />
      )}
